...
panel_keypair = panel_keypair = {"ID" : "wkdtngur"}
            ...
            if ("url" in request.form and "name" in request.form and "adminid" in request.form and "adminpw" in request.form and "adminpwcheck" in request.form and request.form["adminpw"] == request.form["adminpwcheck"] and "license" in request.form):
                if (len(request.form["adminid"]) >= 6 and len(request.form["adminid"]) <= 24 and len(request.form["adminpw"]) >= 6 and len(request.form["adminpw"]) <= 24 and len(request.form["name"]) >= 1 and len(request.form["name"]) <= 12 and request.form["name"].isalpha() and len(request.form["url"]) >= 3 and len(request.form["url"]) <= 12):
                    if not (os.path.isfile(db(request.form["url"]))):
                        captcha_secret = "https://www.google.com/recaptcha/enterprise.js?render=6LdGMYclAAAAADV1lKLLxIRefH8alx8AAt6xixk-"
                        captcha_result = requests.get("https://www.google.com/recaptcha/enterprise.js?render=6LdGMYclAAAAADV1lKLLxIRefH8alx8AAt6xixk-" + request.form["g-recaptcha-response"] + "&remoteip=" + getip()).json()
                        if (captcha_result["success"] == True):
                            ...
                            with con:
                                ...
                                con.commit()
                                cur.execute("""CREATE TABLE "products" ("id" TEXT, "name" TEXT, "description" TEXT, "Nprice" INTEGER, "url" TEXT, "stock" TEXT, "Vprice" INTEGER, "Rprice" INTEGER);""")
                                con.commit()
                                cur.execute("""CREATE TABLE "users" ("id" TEXT, "pw" TEXT,"ip" TEXT, "money" INTEGER, "buylog" TEXT, "isadmin" INTEGER, "black" TEXT, "name" TEXT, "tag" TEXT, "shopURL" TEXT);""")
                                con.commit()
                                cur.execute("""CREATE TABLE "bankwait" ("id" TEXT, "name" TEXT, "amount" INTEGER, "day" TEXT);""")
                                con.commit()
                                cur.execute("""INSERT INTO info VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (request.form["name"], "", "", "", "[]", "[]", "", make_expiretime(license_result[1]), "", "공지가 없습니다.", 0, "", "", 0 if license_result[5] == 0 else 1, "", "https://cdn.discordapp.com/attachments/754331422818435083/795524062482792458/608a38b94a378eff.gif", "", "", "사용함", "", "", "", ""))
                                con.commit()
                                cur.execute("INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (request.form["adminid"], hash(request.form["adminpw"]), getip(), 0, "[]", 1, "", "", "", request.form["url"]))
                                con.commit()
                            con.close()

                            api_key = "qteu2wE-1SgXpNn3U7oANRQ5V8SJDTcYa_UARg74"
                            email = "nameyoutub1@gmail.com"
                            zone_id = "0916b5eae03cf89636cca8253e7d0ef1"
                            ...
                            if (user_info[1] == hash(request.form["pw"])):
                                if (user_info[6] == ""):
                                    ...
                                    if (server_info[19] != None):
                                        try:
                                            webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[19])
                                            embed = DiscordEmbed(description=f'[!] 로그인 알림\n서버이름: {server_info[0]}\n아이디: {request.form["id"]}', color=0x329da8)
                        ...
                        if (user_info == None):
                            ...
                            if (iplist == None):
                                if ((len(request.form["id"]) >= 6 and len(request.form["id"]) <= 24) and (len(request.form["pw"]) >= 6 and len(request.form["pw"]) <= 24)):
                                    con = sqlite3.connect(db(name))
                                    cur = con.cursor()
                                    cur.execute("INSERT INTO users VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (request.form["id"], hash(request.form["pw"]), getip(), 0, "[]", 0, "", "", request.form["tag"], "일반"))
                                    con.commit()
                                    con.close()
        ...
        if (os.path.isfile(db(name))):
            if (name in session):
                ...
                if (str(info[6]) != ""):
                    ...
                    con.close()
                    return render_template("index.html", rank = user_info[9], name=server_info[0], products=products, user_info=user_info, music=server_info[8], shopinfo=server_info, linking=server_info[14], url=name, file=info[16], channelio=server_info[21])
            else:
                return redirect("login")
        else:
            abort(404)
                        ...
                        if (prod_info[5] != "" and str(request.get_json()["amount"]).isdigit() and request.get_json()["amount"] > 0 and len(prod_info[5].split("\n")) >= request.get_json()["amount"]):
                            ...
                            if (int(user_info[3]) >= total_price):
                                ...
                                try:
                                    ...
                                    if (name == "dench"):
                                        webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[1])
                                        embed = DiscordEmbed(description="`" + user_name + "`님, `" + server_info[0] + "`에서 `" + prod_info[1] + "` " + prod_amount+"개 구매 감사합니다! :tada:", color=0x329da8)
                                        webhook.add_embed(embed)
                                        webhook.execute()
                                    else:
                                        webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[1])
                    ...
                    if ("code" in request.get_json()):
                        ...
                        if (culture_id != "" and culture_pw != ""):
                            try:
                                ...
                                json_data = {"token" : "rftGDNoFOZ3LIPapUYUb", "id" : culture_id, "pw" : culture_pw, "pin" : request.get_json()["code"]}
                                try:
                                    ms_result = requests.post("https://munsangjacung.nameyoutub1.repl.co/api/charge", json=json_data)
                                ...
                                if (ms_result["result"] == True):
                                    user_info = search_user(name, session[name])
                                    culture_amount = int(ms_result["amount"])
                                    ...
                                    server_info = get_info(name)
                                    chargelog = ast.literal_eval(server_info[5])
                                    chargelog.append([nowstr(), session[name], request.get_json()["code"], ms_result["reason"], "0"])
                                    con = sqlite3.connect(db(name))
                                    cur = con.cursor()
                                    cur.execute("UPDATE info SET chargelog = ?", (str(chargelog),))
                                    con.commit()
                                    con.close()
                                    return ms_result["reason"]
                            except:
                                pass
                        else:
                    ...
                    if ("name" in request.get_json() and "amount" in request.get_json() and request.get_json()["amount"].isdigit()):
                        ...
                        if (bank_addr != ""):
                            ...
                            if (server_info[19] != None):
                                ...
                                jsondata = {
                                    "api_key": "", "bankpin": str(server_info[12]), "shop": user_info[10], "userinfo": chargereq_info[1], "userid": session[name], "token": "token", "type": True, "amount": int(amount)
                                }
                                print(jsondata)

                                ms_result = requests.post(
                                    "http://127.0.0.1:4040/api", json=jsondata)
                        ...
                        if ("name" in request.form and "price" in request.form):
                            if (request.form["price"].isdigit() and int(request.form["price"]) > 0 and int(request.form["price"]) <= 10000000):
                                ...
                                cur.execute("INSERT INTO products VALUES(?, ?, ?, ?, ?, ?, ?, ?);",
                                            (new_prodid, request.form["name"], "", request.form["price"], "", "", request.form["price2"], request.form["price3"]))
                                con.commit()
                                con.close()
                                return "ok"
                            else:
                            ...
                            if (user_info != None):
                                if (request.form["name"] != ""):
                                    if (request.form["price"].isdigit() and int(request.form["price"]) > 0 and int(request.form["price"]) <= 10000000):
                                        ...
                                        con = sqlite3.connect(db(name))
                                        cur = con.cursor()
                                        cur.execute("UPDATE products SET name = ?, description = ?, Nprice = ?, url = ?, stock = ?, Vprice = ?, Rprice = ? WHERE id == ?", (request.form["name"], request.form["description"], request.form["price"], request.form["photo"], request.form["stock"], request.form["price2"], request.form["price3"], request.form["id"]))
                                        con.commit()
                                        con.close()
                                                ...
                                                if (server_info[20] != None):
                                                    try:
                                                        if (name == "dench"):
                                                            ...
                                                            webhook = DiscordWebhook(username="핵디 계정전문샵", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[20])
                                                            embed = DiscordEmbed(title="[구매하기]", url=f"http://vipvend.kro.kr/{name}/shop", description=f"[제품이름]\n`{names}`\n[제품가격]\n`{prod_info[3]}`원\n[입고전 재고 갯수]\n`0`\n[입고된 재고 갯수]\n`{laststock}`개\n[남은 재고]\n`{laststock}`개", color=0x00fbff)
                                                            ...
                                                            names = request.form["name"]
                                                            webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[20])
                                                            embed = DiscordEmbed(title="[구매하기]", url=f"http://vipvend.kro.kr/{name}/shop", description=f"[제품이름]\n`{names}`\n[제품가격]\n`{prod_info[3]}`원\n[입고전 재고 갯수]\n`0`\n[입고된 재고 갯수]\n`{laststock}`개\n[남은 재고]\n`{laststock}`개", color=0x00fbff)
                                                            embed.set_author(name="[!] 입고알림", icon_url=prod_info[4])
                                                            embed.set_thumbnail(url=prod_info[4])
                                                            embed.set_footer(text=datetime.datetime.now().strftime('%m월 %d일 %H시 %M분'.encode('unicode-escape').decode()).encode().decode('unicode-escape'))
                                                ...
                                                if (server_info[20] != None):
                                                    try:
                                                        if (name == "dench"):
                                                            names = request.form["name"]
                                                            webhook = DiscordWebhook(username="핵디 계정전문샵", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url=server_info[20])
                                                            embed = DiscordEmbed(title="[구매하기]", url=f"http://vendvip.kro.kr/{name}/shop", description=f"[제품이름]\n`{names}`\n[제품가격]\n`{prod_info[3]}`원\n[입고전 재고 갯수]\n`{nowstock}`\n[입고된 재고 갯수]\n`{laststock - nowstock}`개\n[남은 재고]\n`{laststock}`개", color=0x00fbff)
                                                            embed.set_author(name="[!] 입고알림", icon_url=prod_info[4])
                                                            embed.set_thumbnail(url=prod_info[4])
                                                            embed.set_footer(text=datetime.datetime.now().strftime('%m월 %d일 %H시 %M분'.encode('unicode-escape').decode()).encode().decode('unicode-escape'))
                                                            webhook.add_embed(embed)
                    ...
                    if (password != "" and password == request.get_json()["pw"]):
                        def process_post(name, amount, url):
                            ...
                            if (chargeinfo_detail != None):
                                ...
                                webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url="https://discord.com/api/webhooks/826432652885360640/TODpt88j2i1tNLljqwJk7PC06CvwkZH89VxQeg_phbrvkGtgiBNUVFx9S1eI-A6zKxLE")
                                embed = DiscordEmbed(description=f"[!] BANK POST complete : {name}, {amount} KRW", color=0x329da8)
                                webhook.add_embed(embed)
                                webhook.execute()
                        ...
                        webhook = DiscordWebhook(username="VipVend", avatar_url="https://i.pinimg.com/736x/8a/f0/c6/8af0c6ec26d3778b2f67a9bced6ee875.jpg", url="https://discord.com/api/webhooks/826432652885360640/TODpt88j2i1tNLljqwJk7PC06CvwkZH89VxQeg_phbrvkGtgiBNUVFx9S1eI-A6zKxLE")
                        embed = DiscordEmbed(description=f"[!] BANK POST : {str(request.get_json())}", color=0x329da8)
                        webhook.add_embed(embed)
                        webhook.execute()
